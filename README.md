...
`memory_write` 写入后会自动做一次**疑似重复/冲突检测**（基于字符 bigram 的 Jaccard 相似度，零依赖、对中文天然友好），命中时在返回值末尾追加提示：

- **标题相似度 ≥ 0.6** → 「疑似重复标题」
- **正文相似度 ≥ 0.5**（且标题不相似）→ 「疑似重复/冲突内容」
...
- **290 条断言**的九层测试：语义回归 28 + 全工具冒烟 25 + 记忆可信度 24 + 作用域与生命周期 70 + 跨进程并发 38 + 异常恢复 35 + 检索质量 7 + 索引链接可解析性 18 + 运行时可观测性 45
...
测试分四类：**正确性**（写读是否无损）、**覆盖面**（工具是否都能用）、**语义与质量**（字段标记是否可靠、检索是否退化）、**韧性**（并发、崩溃、损坏文件下是否还站得住）。共 290 条断言。
...
`记忆索引.md` 由 `_refresh_index()` 在每次 `memory_write` 后自动重建。Obsidian 解析双链以**文件名 stem** 为准，而 `safe_filename()` 会把标题里的空格等字符归一化成下划线 —— 若生成器直接输出 `[[title]]`，凡标题含空格的笔记在索引里全是渲染死链，且每次写入都会把人工修复覆盖回去。

判定口径是一句话：**索引与索引草稿里的每一条链接，其 target 都必须能解析到库中真实存在的文件**。覆盖：

- 标题==stem 时输出裸链接、标题!=stem 时 target 用 stem 且标题进别名位
